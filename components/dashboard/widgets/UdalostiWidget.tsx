        ...
        {events.map((ev) => {
          ...
          return (
            <Link key={ev.id} href={`/udalosti/${ev.id}`} className="group flex items-start gap-3 py-2 first:pt-0 last:pb-0">
              <div className="w-10 h-10 rounded-base bg-amber-100 flex flex-col items-center justify-center flex-shrink-0">
                <span className="font-data text-sm font-bold text-amber-700 leading-none">{day}</span>
                <span className="text-[10px] uppercase text-amber-700/70 leading-none mt-0.5">{month}</span>
              </div>
