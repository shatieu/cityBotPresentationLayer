    ...
    >
      <div className="space-y-2.5">
        {activities.map((a) => (
          <Link key={a.id} href={`/krouzky/${a.id}`} className="block group">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              {a.category && <Badge variant="stone">{a.category}</Badge>}
              {a.schedule && (
                <span className="font-data text-xs text-gold-700 bg-gold-50 px-1.5 py-0.5 rounded-sm">
                  {a.schedule}
                </span>
              )}
            </div>
        ...
        {activities.length === 0 && !loading && (
          <p className="text-sm text-stone-500">Žádné kroužky.</p>
        )}
      </div>
    </WidgetCard>
  );
