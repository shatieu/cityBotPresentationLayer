      ...
      <div className="space-y-3">
        {shows.map((show) => (
          <div key={show.id} className="flex items-start gap-3">
            <div className="w-8 h-8 rounded-base bg-wine-100 flex items-center justify-center flex-shrink-0 mt-0.5">
              <Theater size={14} className="text-wine-700" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium text-ink-900">{show.title}</p>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="font-data text-xs font-medium text-gold-700">
                  {formatCzechDate(show.date_time)} {formatTime(show.date_time)}
                </span>
                {show.genre && (
                  <Badge variant="wine">{show.genre}</Badge>
                )}
              </div>
        ...
        {shows.length === 0 && !loading && (
          <p className="text-sm text-stone-500">Žádná představení v nejbližších dnech.</p>
        )}
      </div>
    </WidgetCard>
  );
